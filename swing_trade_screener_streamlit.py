...
def build_price_chart_image(t: Dict[str, Any], symbol: str) -> bytes:
    """Static PNG version of the app's 5-panel chart (Price/EMA/Bollinger/VWAP,
    Volume, RSI+Stochastic, MACD, ADX/DI), for embedding in the PDF report so
    the PDF mirrors what's shown in the on-screen detail view."""
    c = t["chart"]
    n = len(c["dates"])
    ...
    fig, axes = plt.subplots(
        5, 1, figsize=(9.5, 12), sharex=True,
        gridspec_kw={"height_ratios": [3.2, 1.1, 1.3, 1.3, 1.1], "hspace": 0.12},
    )
    ax_price, ax_vol, ax_rsi, ax_macd, ax_adx = axes

    # --- Price + EMA + Bollinger + VWAP (candlesticks) ---
    width = (x[-1] - x[0]) / max(n, 1) * 0.6 if n > 1 else 0.6
    for i in range(n):
        color = "#16A34A" if c["close"][i] >= c["open"][i] else "#DC2626"
        ax_price.plot([x[i], x[i]], [c["low"][i], c["high"][i]], color=color, linewidth=0.7, zorder=2)
        y0, y1 = sorted([c["open"][i], c["close"][i]])
        ax_price.add_patch(plt.Rectangle((x[i] - width / 2, y0), width, max(y1 - y0, 0.01),
                                          color=color, zorder=3))
    ax_price.plot(x, c["ema20"], color="#2563EB", linewidth=1, label="EMA20")
    ax_price.plot(x, c["ema50"], color="#EA580C", linewidth=1, label="EMA50")
    if c.get("ema200"):
        ax_price.plot(x, c["ema200"], color="#7C3AED", linewidth=1, label="EMA200")
    ax_price.plot(x, c["vwap"], color="#64748B", linewidth=0.8, linestyle=":", label="VWAP")
    ax_price.plot(x, c["bb_upper"], color="#16A34A", linewidth=0.6, alpha=0.5)
    ax_price.plot(x, c["bb_lower"], color="#16A34A", linewidth=0.6, alpha=0.5)
    ax_price.fill_between(x, c["bb_lower"], c["bb_upper"], color="#16A34A", alpha=0.05)
    ax_price.set_title(f"{symbol} — Price / EMA / Bollinger / VWAP", fontsize=10, loc="left")
    ax_price.legend(fontsize=7, loc="upper left", ncol=4, frameon=False)
    ax_price.tick_params(labelsize=7)

    # --- Volume ---
    vol_colors = ["#16A34A" if c["close"][i] >= c["open"][i] else "#DC2626" for i in range(n)]
    ax_vol.bar(x, c["volume"], color=vol_colors, width=width)
    ax_vol.plot(x, c["vol_avg20"], color="#0F172A", linewidth=0.8)
    ax_vol.set_title("Volume", fontsize=9, loc="left")
    ax_vol.tick_params(labelsize=7)

    # --- RSI + Stochastic ---
    ax_rsi.plot(x, c["rsi"], color="#0EA5E9", linewidth=1, label="RSI")
    ax_rsi.plot(x, c["stoch_k"], color="#F59E0B", linewidth=0.8, label="Stoch %K")
    ax_rsi.plot(x, c["stoch_d"], color="#EA580C", linewidth=0.8, linestyle=":", label="Stoch %D")
    ax_rsi.axhline(70, color="#DC2626", linewidth=0.6, linestyle=":")
    ax_rsi.axhline(30, color="#16A34A", linewidth=0.6, linestyle=":")
    ax_rsi.set_title("RSI (14) + Stochastic", fontsize=9, loc="left")
    ax_rsi.legend(fontsize=7, loc="upper left", ncol=3, frameon=False)
    ax_rsi.tick_params(labelsize=7)

    # --- MACD ---
    ax_macd.bar(x, c["macd_hist"], color="#94A3B8", width=width)
    ax_macd.plot(x, c["macd"], color="#2563EB", linewidth=1, label="MACD")
    ax_macd.plot(x, c["macd_signal"], color="#EA580C", linewidth=1, label="Signal")
    ax_macd.set_title("MACD", fontsize=9, loc="left")
    ax_macd.legend(fontsize=7, loc="upper left", ncol=2, frameon=False)
    ax_macd.tick_params(labelsize=7)

    # --- ADX / DI ---
    ax_adx.plot(x, c["adx"], color="#0F172A", linewidth=1, label="ADX")
    ax_adx.plot(x, c["plus_di"], color="#16A34A", linewidth=0.8, label="+DI")
    ax_adx.plot(x, c["minus_di"], color="#DC2626", linewidth=0.8, label="-DI")
    ax_adx.axhline(25, color="#64748B", linewidth=0.6, linestyle=":")
    ax_adx.set_title("ADX / DI", fontsize=9, loc="left")
    ax_adx.legend(fontsize=7, loc="upper left", ncol=3, frameon=False)
    ax_adx.tick_params(labelsize=7)
    ax_adx.xaxis.set_major_formatter(mdates.DateFormatter("%b %y"))
    ax_adx.xaxis.set_major_locator(mdates.AutoDateLocator())
    fig.autofmt_xdate()
...
def build_stock_report(candidate: dict) -> bytes:
    ...
    h2 = ParagraphStyle("H2", parent=styles["Heading2"], fontSize=13, textColor=colors.HexColor("#334155"))
    body = ParagraphStyle("Body", parent=styles["BodyText"], fontSize=9, leading=12)
    small = ParagraphStyle("Small", parent=styles["BodyText"], fontSize=8, textColor=colors.HexColor("#64748B"))

    story = []
    story.append(Paragraph("Swing Trade Screener Pro — Unified Engine", small))
    story.append(Paragraph(
        f"{candidate.get('name','')} <font color='#64748B'>({candidate.get('base_symbol','')})</font>", h1,
    ))
    story.append(Paragraph(
        f"Sector: {candidate.get('sector','—')} | Industry: {candidate.get('industry','—')} | "
    ...
    summary_data = [
        ["Price (INR)", _fmt(price, "₹"), "Change", _fmt(change, "", "%")],
        ["Technical Score", f"{t_score}/100", "Fundamental Score", f"{f_score}/100"],
        ["Setups", ", ".join(setups), "Market Cap", _fmt((candidate.get("market_cap") or 0) / 1e7, "₹", " Cr", 0) if candidate.get("market_cap") else "—"],
    ]
    tbl = Table(summary_data, colWidths=[42 * mm, 42 * mm, 42 * mm, 42 * mm])
    ...
    # --- Chart (mirrors the on-screen detail-view chart) ---
    t_for_chart = candidate.get("technical", {}) or {}
    if t_for_chart.get("chart"):
        try:
            chart_png = build_price_chart_image(t_for_chart, candidate.get("base_symbol", ""))
            img = RLImage(BytesIO(chart_png), width=180 * mm, height=180 * mm * (12 / 9.5))
            story.append(img)
            story.append(Spacer(1, 10))
        except Exception:
    ...
    plan_data = [
        ["Entry", _fmt(plan.get("entry"), "₹"), "Stop Loss", _fmt(plan.get("stop_loss"), "₹")],
        ["Target", _fmt(plan.get("target"), "₹"), "Risk : Reward", _fmt(plan.get("risk_reward"), "1 : ", "", 2)],
        ["Risk / Share", _fmt(plan.get("risk_per_share"), "₹"), "Reward / Share", _fmt(plan.get("reward_per_share"), "₹")],
    ]
    tbl2 = Table(plan_data, colWidths=[42 * mm, 42 * mm, 42 * mm, 42 * mm])
    ...
    story.append(Paragraph("Technical Snapshot", h2))
    tech_data = [
        ["EMA20", _fmt(t.get("ema20"), "₹"), "EMA50", _fmt(t.get("ema50"), "₹"), "EMA200", _fmt(t.get("ema200"), "₹")],
        ["RSI(14)", _fmt(t.get("rsi")), "MACD Hist", _fmt(t.get("macd_hist"), "", "", 4), "VWAP", _fmt(t.get("vwap"), "₹")],
        ["Support", _fmt(t.get("support"), "₹"), "Resistance", _fmt(t.get("resistance"), "₹"), "ATR(14)", _fmt(t.get("atr"), "₹")],
        ["Volume", _fmt(t.get("volume"), "", "", 0), "Avg Vol (20d)", _fmt(t.get("avg_volume_20"), "", "", 0), "Vol Ratio", _fmt(t.get("volume_ratio"), "", "x")],
        ["ADX(14)", _fmt(t.get("adx")), "Stoch %K", _fmt(t.get("stoch_k")), "Stoch %D", _fmt(t.get("stoch_d"))],
        ["BB Upper", _fmt(t.get("bb_upper"), "₹"), "BB Lower", _fmt(t.get("bb_lower"), "₹"), "BB Width%", _fmt(t.get("bb_width_pct"), "", "%")],
    ]
    tbl3 = Table(tech_data, colWidths=[28 * mm] * 6)
    ...
    signals = []
    if t.get("patterns"):
        signals.append(f"Candlestick: {', '.join(t['patterns'])}")
    if t.get("divergence"):
        signals.append(f"Divergence: {t['divergence']}")
    if t.get("gap"):
        signals.append(f"Gap: {t['gap']}")
    signals.append(f"Weekly trend up: {t.get('weekly_trend_up')}")
    signals.append(f"Monthly trend up: {t.get('monthly_trend_up')}")
    story.append(Paragraph("Advanced Signals", h2))
    for s in signals:
    ...
    fund_data = [
        ["ROE", _fmt(f.get("roe_pct"), "", "%"), "PE", _fmt(f.get("pe")), "EPS", _fmt(f.get("eps"))],
        ["Revenue Growth", _fmt(f.get("revenue_growth_pct"), "", "%"), "Earnings Growth", _fmt(f.get("earnings_growth_pct"), "", "%"), "Profit Margin", _fmt(f.get("profit_margin_pct"), "", "%")],
        ["Operating Margin", _fmt(f.get("operating_margin_pct"), "", "%"), "Debt / Equity", _fmt(f.get("debt_to_equity_val")), "Current Ratio", _fmt(f.get("current_ratio"))],
    ]
    tbl4 = Table(fund_data, colWidths=[28 * mm] * 6)
